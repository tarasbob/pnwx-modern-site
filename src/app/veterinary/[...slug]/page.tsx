...
export async function generateStaticParams() {
  return (veterinaryCategory.subcategories ?? []).map((sub) => ({
    slug: [sub.slug],
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const sub = veterinaryCategory.subcategories?.find((s) => s.slug === slug[0]);
  if (!sub) return {};
  return { title: sub.seoTitle, description: sub.seoDescription };
}

export default async function VeterinarySlugPage({ params }: Props) {
  const { slug } = await params;
  ...
  const breadcrumbs = [
    ...
    { label: sub.name, href: `/veterinary/${sub.slug}` },
    ...(slug.length > 1 ? [{ label: name, href: `/veterinary/${slug.join("/")}` }] : []),
  ];
