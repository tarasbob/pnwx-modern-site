...
export async function generateStaticParams() {
  return (suppliesCategory.subcategories ?? []).map((sub) => ({
    slug: [sub.slug],
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const sub = suppliesCategory.subcategories?.find((s) => s.slug === slug[0]);
  if (!sub) return {};
  return { title: sub.seoTitle, description: sub.seoDescription };
}

export default async function SuppliesSlugPage({ params }: Props) {
  const { slug } = await params;
  ...
  const breadcrumbs = [
    ...
    { label: sub.name, href: `/supplies/${sub.slug}` },
    ...(slug.length > 1 ? [{ label: name, href: `/supplies/${slug.join("/")}` }] : []),
  ];
