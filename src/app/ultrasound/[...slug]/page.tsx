...
export async function generateStaticParams() {
  return (ultrasoundCategory.subcategories ?? []).map((sub) => ({
    slug: [sub.slug],
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const sub = ultrasoundCategory.subcategories?.find((s) => s.slug === slug[0]);
  if (!sub) return {};
  return { title: sub.seoTitle, description: sub.seoDescription };
}

export default async function UltrasoundSlugPage({ params }: Props) {
  const { slug } = await params;
  ...
  const breadcrumbs = [
    ...
    { label: sub.name, href: `/ultrasound/${sub.slug}` },
    ...(slug.length > 1 ? [{ label: name, href: `/ultrasound/${slug.join("/")}` }] : []),
  ];
